...
"""
API For QGIS Server custom GetProjectSettings request
"""
...
class WebMapService_QGIS(object):
...
class ContentMetadata(AbstractContentMetadata):
    ...
    def __init__(self, elem, parent=None, children=None, index=0, parse_remote_metadata=False,
        ...
        self.visible = int(elem.attrib.get('visible', 0))
        self.geometryType = elem.attrib.get('geometryType', None)
        ...
        featureattributes = []
        attr = elem.find(nspath('Attributes', WMS_NAMESPACE))
        if attr is not None:
            for attribute in attr.findall(nspath('Attribute', WMS_NAMESPACE)):
                name = attribute.attrib.get('name', None)
                featureattributes.append(name)
        self.Attributes = featureattributes

        # Primary Key
        p = elem.find(nspath('PrimaryKey', WMS_NAMESPACE))
        self.PrimaryKeyAttribute = None
        if p is not None:
            self.PrimaryKeyAttribute = p.find(nspath('PrimaryKeyAttribute', WMS_NAMESPACE)).text
